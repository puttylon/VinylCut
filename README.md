...
Steuer-Skript für die Songtexte-Pipeline (Architektur siehe `workflow für songexte.txt`, Abschnitt „ZIELARCHITEKTUR"; Baufortschritt siehe `ROADMAP.md`, „Songtexte-Pipeline-Umbau"). Jeder Schritt hat sein eigenes Flag — kein Sammel-Flag `--phase` mehr (Nutzer-Feedback: „kein Mensch braucht im Flag den Begriff 'phase'"). Jeder Schritt liest/schreibt ausschließlich über die gemeinsame Cache-DB (`cache.db`, siehe unten) und ist einzeln wiederholbar:
...
**Mit PFAD läuft die Pipeline Datei für Datei:** Der Verzeichnis-Walk + das Tag-Lesen laufen dabei lazy (in Dateinamen-/Verzeichnisreihenfolge) — jede Datei durchläuft alle gewählten Schritte komplett (`scan` → `abfragen`/`nachholen` → `bewerten` → `schreiben`), SOBALD sie gefunden ist, bevor die nächste Datei überhaupt gesucht wird. Nicht mehr Ordner für Ordner, und kein Warten mehr auf den kompletten Baum, bevor der erste Track verarbeitet wird — bei einer großen, netzwerk-gemounteten Bibliothek wirkte das vorher wie ein Hänger. Bewusster Trade-off: es gibt deshalb keine "N Datei(en) gefunden."-Zeile vorab, die Gesamtzahl ist erst am Ende bekannt. Das schiebt außerdem spürbaren Leerlauf zwischen zwei Anbieter-Abfragen (`--bewerten` kann pro Song mehrere Minuten Live-Whisper-Transkription brauchen), was das Risiko eines Anbieter-Rate-Limits senkt — ganz ohne eigene Wartezeit-Logik. Bei einem Abbruch mitten im Lauf sind bereits fertige Dateien schon geschrieben. Ohne PFAD (z.B. `--nachholen` allein) läuft weiterhin alles global in einem Rutsch über die ganze Cache-DB, da es dort keine Datei-Struktur gibt.
...
Die vier Anbieter `lrclib`, `musixmatch`, `netease` und `genius` werden gleichzeitig befragt (je max. 20 s Timeout). Artist und Titel kommen aus den Audio-Tags. Klammer-Zusätze im Titel (`(Live In Osaka Japan 16th August 1972)`, `[Deluxe Edition 2014 Remix]` u.ä.) werden für den Suchbegriff entfernt — Lyrics-Provider kennen meist nur den Kern-Titel, der Songtext ist ohnehin identisch zur Studio-Version. Die Suchanfrage selbst wird zusätzlich ASCII-gefaltet (Akzent-/Sonderzeichen wie ein alleinstehendes „´" → normaler Apostroph, „é" → „e") — Tag-Artefakte lassen die Provider-Suche sonst manchmal ins Leere laufen, obwohl der Song dort unter der üblichen Schreibweise existiert. Title-Tag, Dateiname und die Cache-Schlüssel (Künstler/Titel) bleiben davon unberührt, nur die Live-Suchanfrage ist betroffen.

**Provider-Konsens (Schnellweg)** (Schritt `--bewerten`)

Bis zu 5 Kandidaten fließen ein: die 4 frischen Provider-Antworten plus eine bereits vorhandene `.lrc`-Datei (falls der Track schon mal bewertet wurde). Kommen davon mindestens 3 mit auswertbarem Text zusammen, wird ein Konsens versucht. Inhaltlich (wort-basiert, nicht nur byteweise) nahezu identische Kandidaten — z. B. gespiegelte Provider-Datenbanken oder eine bestehende Datei gegenüber einem frischen Re-Fetch desselben Anbieters — werden vorher zu einer Gruppe zusammengefasst und zählen nur als eine Stimme, damit sie die eigentliche Übereinstimmungs-Rechnung nicht verfälschen (verhindert, dass eine mehrfach gespiegelte falsche Quelle wie mehrere unabhängige Bestätigungen wirkt). Stimmen die verbliebenen Gruppen untereinander zu mindestens 40 % überein (Jaccard-Ähnlichkeit), gilt das als Konsens — auch wenn dabei z. B. nur noch 2 Gruppen übrig sind, weil sich vier von fünf Quellen zu einer zusammengefasst haben: die ursprüngliche Quellenzahl entscheidet über die Mindestanzahl, nicht die Gruppenzahl danach. Der repräsentativste Kandidat — also der mit der höchsten Durchschnitts-Ähnlichkeit zu allen anderen — wird ohne Whisper-Prüfung gespeichert. Ausreißer (falscher Song, andere Sprache, veraltete lokale Datei) werden dadurch automatisch übergangen, auch wenn es die bereits vorhandene Datei selbst ist.
...
Whisper transkribiert den Track (maximal 8 Minuten) — bricht aber früh ab (Early-Stop), sobald ein Kandidat über mehrere Checkpoints stabil sicher erkannt ist; nur der Accept-Fall wird beschleunigt, Ablehnung/Instrumental-Erkennung laufen immer am vollen Transkript (siehe `ROADMAP.md`). Die Pro-Track-Zeile zeigt dann zusätzlich `früh-gestoppt`. Das Modell richtet sich nach der erkannten Sprache: englischsprachige Songs nutzen `medium`, alle anderen (insbesondere Deutsch und gemischtsprachige Songs) `large-v3` — der Qualitätsgewinn von `large-v3` ist dort laut Testlauf real und deutlich, bei Englisch dagegen zu gering für dessen Mehrkosten (siehe `ROADMAP.md`).

Der Text wird mit jeder Provider-LRC verglichen. Das Ähnlichkeitsmaß ist **IDF-gewichtetes Jaccard**: `Σ idf(w) für w in (Whisper ∩ LRC) ÷ Σ idf(w) für w in (Whisper ∪ LRC)`. Seltene, inhaltstragende Wörter zählen dabei stark, häufige Stopwords kaum — das verhindert Fehlmatches, bei denen zufällig ein paar generische Wörter übereinstimmen, obwohl der Song nicht passt. Die IDF-Werte (Dokumentfrequenz je Wort) stammen aus einer **globalen, aus der Cache-DB gebauten Tabelle** (`cache.db`, siehe unten) — keine separate Datei, keine Sprach-Teiltabellen.
...
**`db_analyse.py`** — Gegenstück zu `lrc_analyse.py`: wertet nicht den JSON-Ordner-Cache aus, sondern direkt die SQLite-Cache-DB (`cache.db`):

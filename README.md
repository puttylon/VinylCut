...
**`songtext_pipeline.py`** — Steuer-Skript für die Phasen-Pipeline, die `fetch_songtext.py` künftig ablösen soll (Architektur siehe `workflow für songexte.txt`, Abschnitt „ZIELARCHITEKTUR", Baufortschritt siehe `ROADMAP.md`, „Songtexte-Pipeline-Umbau"). **Alle 5 Phasen laufen echt:** `--phase 1` (scannen: Tags lesen, Song-Identität in der Cache-DB anlegen — auch als eigenständiges Modul `scan_songs.py` nutzbar), `--phase 2`/`--phase 3` (Anbieter abfragen bzw. Nachhol-Modus für fehlgeschlagene/fehlende Provider-Treffer — auch als eigenständiges Modul `fetch_providers.py` nutzbar, überspringt dabei Songs mit Skip-Genre wie Hörbuch/Hörspiel/Instrumental), `--phase 4` (bewerten: Konsens-Prüfung, sonst Whisper-Verifikation — auch als eigenständiges Modul `evaluate_lyrics.py` nutzbar; wählt automatisch `medium` für englische bzw. `large-v3` für nicht-englische Songs), `--phase 5` (`.lrc` schreiben/löschen je nach Phase-4-Entscheidung, JSON-Ordner-Cache pflegen — auch als eigenständiges Modul `write_lrc.py` nutzbar). Ohne PFAD arbeiten Phase 2/3/4 bewusst über die GANZE Bibliothek (explizite „alles nachziehen"-Absicht), mit PFAD nur über die Songs des aktuellen Laufs — Phase 3 wird bei gesetztem PFAD automatisch übersprungen (läuft nur ohne PFAD). Phase 1/4/5 brauchen für die Datei-Zuordnung/den Whisper-Zugriff/das Schreiben zusätzlich eine echte Audiodatei.

```bash
python3 songtext_pipeline.py "/Musik/" --recursive        # alle 5 Phasen nacheinander (Phase 3 wird dabei übersprungen)
python3 songtext_pipeline.py "/Musik/" --phase 2,4,5      # nur ausgewählte Phasen
python3 songtext_pipeline.py --phase 3                    # nur Nachhol-Modus über die ganze Bibliothek — PFAD nicht nötig
```

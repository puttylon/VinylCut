...
Whisper transkribiert den Track (maximal 8 Minuten) — bricht aber früh ab (Early-Stop), sobald ein Kandidat über mehrere Checkpoints stabil sicher erkannt ist; nur der Accept-Fall wird beschleunigt, Ablehnung/Instrumental-Erkennung laufen immer am vollen Transkript (siehe `ROADMAP.md`). Symmetrisch dazu bricht die Transkription auch ab, wenn der Score gegen ALLE Kandidaten über mehrere Checkpoints praktisch bei Null bleibt (Whisper-Halluzination mit falscher Sprache, z. B. kyrillischer Text bei einem englischen Song) — das ist keine frühe Ablehnung, nur ein vorzeitiges Ende der Transkription (wie beim 300s-Wall-Clock-Deckel); die eigentliche Annahme-/Ablehnungsentscheidung läuft unverändert über das gesammelte Teilstück. Die Pro-Track-Zeile zeigt in beiden Fällen zusätzlich `früh-gestoppt`. Das Modell richtet sich nach der erkannten Sprache: englischsprachige Songs nutzen `medium`, alle anderen (insbesondere Deutsch und gemischtsprachige Songs) `large-v3` — der Qualitätsgewinn von `large-v3` ist dort laut Testlauf real und deutlich, bei Englisch dagegen zu gering für dessen Mehrkosten (siehe `ROADMAP.md`).

...
**Fünfter Kandidat „lokal" (bei aktivem Cache):** Ein früher für denselben Song akzeptierter Text (Quelle `"lokal"` im Cache, siehe `CACHE_DESIGN.md`) wird als zusätzlicher Kandidat einbezogen — aber kein unabhängiger Provider. Er wird ebenfalls per Inhalt-Hash dedupliziert (niedrigste Priorität: identischer Inhalt eines echten Providers gewinnt), zählt **nicht** zum Provider-Konsens in Schritt 3, landet aber bei Überleben des Dedups zusätzlich in der Whisper-Vergleichsliste. Liefert kein einziger echter Provider etwas und nur „lokal" hat einen Treffer, läuft der Track trotzdem ganz normal durch Whisper (kein Freifahrtschein). Jeder akzeptierte Song (Konsens oder Whisper-Treffer) wird automatisch als neuer „lokal"-Stand zurückgeschrieben — bleibt so dauerhaft aktuell, nicht nur ein einmaliger `cache_seed.py`-Snapshot.
...
**Cache-Modul** (`fetch_songtext_cache.db`, neben dem Skript; Design siehe `CACHE_DESIGN.md`): normalisiertes Schema — `songs` (ein Künstler/Titel = eine Zeile) verknüpft mit `ergebnisse` (ein Versuch je Provider, bis zu vier pro Song), `texte` (jeder Liedtext-Inhalt einmal, per Fingerabdruck dedupliziert) und `transkripte` (ein Whisper-Transkript je Song, gleiche Künstler+Titel-Identität wie `songs` — nicht an Datei/Modell/Parameter gebunden). Speichert erfolgreiche Provider-Antworten (inkl. lokaler `.lrc`, Quelle `"lokal"`, eingelesen per `cache_seed.py <bibliothekspfad>` — nur Tracks, die in der `.fetch_songtext.json` ihres Ordners mit `"r": "ok"` verzeichnet sind, als Qualitätsfilter für nachweislich durchs Tool gelaufene und verifizierte Tracks) und Whisper-Transkripte, damit ein Neuaufbau der Bibliothek nach Code-Änderungen ohne erneute Provider-Abfragen/Whisper-Läufe möglich ist. Grundprinzip: das Skript läuft **immer auch mit leerer oder fehlender Datenbank** exakt wie ohne Cache — der Cache ist nur ein Beschleuniger, nie eine Voraussetzung. Jeder Provider-Versuch wird festgehalten — auch ein Fehlschlag (`status="fehlschlag"` mit `fehlergrund`: `rate_limit`/`captcha`/`timeout`/`gesperrt`), aber ein Fehlschlag zählt nie als gültiger Cache-Treffer (immer erneuter Live-Versuch beim nächsten Lauf). Eskaliert der Rate-Limit-Backoff eines Providers auf 5 Treffer in Folge (z.B. dauerhaft blockiertes Musixmatch-Captcha), wird er für 15 Minuten komplett übersprungen statt bei jedem Song erneut zu warten — der Fehlschlag wird dabei sofort (ohne Live-Versuch) mit `fehlergrund="gesperrt"` festgehalten. Provider-Treffer verfallen nach 30 Tagen (`--cache-ttl`). Mit `--no-cache` komplett deaktivieren; `--refresh-cache` **und** `--force` erzwingen beide eine frische Live-Abfrage (umgehen den Provider-Cache vollständig).

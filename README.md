...
**`songtext_pipeline.py`** — Steuer-Skript für die Phasen-Pipeline, die `fetch_songtext.py` künftig ablösen soll (Architektur siehe `workflow für songexte.txt`, Abschnitt „ZIELARCHITEKTUR", Baufortschritt siehe `ROADMAP.md`, „Songtexte-Pipeline-Umbau"). **Alle 5 Phasen laufen echt**, ansteuerbar über sprechende Namen bei `--phase` (`scan`, `abfragen`, `nachholen`, `bewerten`, `schreiben` — nicht mehr über Zahlen): `scan` (Tags lesen, Song-Identität in der Cache-DB anlegen — auch als eigenständiges Modul `scan_songs.py` nutzbar), `abfragen`/`nachholen` (Anbieter abfragen bzw. Nachhol-Modus für fehlgeschlagene/fehlende Provider-Treffer — auch als eigenständiges Modul `fetch_providers.py` nutzbar, überspringt dabei Songs mit Skip-Genre wie Hörbuch/Hörspiel/Instrumental), `bewerten` (Konsens-Prüfung, sonst Whisper-Verifikation — auch als eigenständiges Modul `evaluate_lyrics.py` nutzbar; wählt automatisch `medium` für englische bzw. `large-v3` für nicht-englische Songs), `schreiben` (`.lrc` schreiben/löschen je nach Bewertung, JSON-Ordner-Cache pflegen — auch als eigenständiges Modul `write_lrc.py` nutzbar). Ohne PFAD arbeiten `abfragen`/`nachholen`/`bewerten` bewusst über die GANZE Bibliothek (explizite „alles nachziehen"-Absicht), mit PFAD nur über die Songs des aktuellen Laufs — `nachholen` wird bei gesetztem PFAD automatisch übersprungen (läuft nur ohne PFAD). `scan`/`bewerten`/`schreiben` brauchen für die Datei-Zuordnung/den Whisper-Zugriff/das Schreiben zusätzlich eine echte Audiodatei.

```bash
python3 songtext_pipeline.py "/Musik/" --recursive                     # alle 5 Phasen nacheinander ("nachholen" wird dabei übersprungen)
python3 songtext_pipeline.py "/Musik/" --phase abfragen,bewerten,schreiben   # nur ausgewählte Phasen
python3 songtext_pipeline.py --phase nachholen                         # nur Nachhol-Modus über die ganze Bibliothek — PFAD nicht nötig
```

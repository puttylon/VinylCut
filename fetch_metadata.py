...
def get_flac_duration(flac_path):
    try:
        return library.get_audio_duration(flac_path)
    except Exception:
        return 0.0
...
def _parse_discogs_duration(s):
    ...
    try:
        return float(
            sum(int(x) * 60**i for i, x in enumerate(reversed(s.strip().split(":"))))
        )
    except ValueError:
        return None
...
def search_musicbrainz(
    artist: str, album: str, flac_total: float, max_results: int = 5
) -> list:
...
def score_release(cand, flac_total, album):
    ...
    title_pen = (
        0.0 if _norm_title(cand.get("title", "")) == _norm_title(album) else 100.0
    )
...
def main():
    ...
    if not plausible:
        ...
        if not all_cands:
            sys.exit(
                "Fehler: Kein passendes Release gefunden (weder Discogs noch MusicBrainz)."
            )
    ...
    for i, res in enumerate(plausible[:DEFAULT_MAX_RELEASES], 1):
        rel_id = res.get("id")
        print(
            f"  > Prüfe Pressung {i}/{min(len(plausible), DEFAULT_MAX_RELEASES)} (ID: {rel_id})..."
        )
        ...
        if score <= 5.0 and cand["is_vinyl"]:
            print(
                f"  ✓ Perfekter Match gefunden (Score: {score:.1f}). Breche weitere Suche ab."
            )
            break

    if not best_cand:
        sys.exit("Fehler: Konnte keine validen Tracks laden.")
    ...
    while True:
        ...
        ans = input(
            "\n[Enter] Akzeptieren, Discogs-ID oder MusicBrainz-ID eingeben: "
        ).strip()
        if not ans:
            break
        ...
        if new_cand:
            ...
            print(
                "✗ Fehler: ID nicht gefunden oder enthält keine validen Tracks. Bitte erneut versuchen."
            )
        ...
        try:
            with urllib.request.urlopen(
                urllib.request.Request(
                    c["cover_url"], headers={"User-Agent": DISCOGS_UA}
                ),
                timeout=20,
            ) as r:
                (out_dir / "cover.jpg").write_bytes(r.read())
                print(
                    f"✓ Cover gespeichert (von '{c['title']}', {c['community_have']} Besitzer)."
                )
            break
        except Exception:
            continue

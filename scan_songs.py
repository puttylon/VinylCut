...
liest Künstler/Titel/Genre-Tags (fetch_songtext._read_audio_tags) und trägt
...
DB-Eintrag, genau wie im bisherigen Verhalten in fetch_songtext.main() (Zeile
~2305).
...
Wird von songtext_pipeline.py für --phase 1 aufgerufen (siehe dort, main()).
"""
...
def _iter_audio_files(root: Path, recursive: bool) -> Iterable[Path]:
    ...
    Wird auch von songtext_pipeline.build_file_song_map wiederverwendet --
    lebt hier, damit songtext_pipeline.py diese Phase importieren kann, ohne
    einen Zirkelimport zu erzeugen.
    """
    if root.is_file():
...
def scan(root: Path, recursive: bool, conn: sqlite3.Connection) -> int:
    ...
    Titel-Normalisierung wie beim bestehenden Anlegen von songs-Zeilen in
    fetch_songtext.py: erst _clean_query_title auf den Titel, dann
    cache_store.normalize_key -- dieselbe Normalisierung wie in
    songtext_pipeline.build_file_song_map, sonst laufen Scan-Ergebnis und
    die dortige Datei-Zuordnung auseinander.
    ...
    for audio_path in _iter_audio_files(root, recursive):
        artist, title, genre = fetch_songtext._read_audio_tags(audio_path)
